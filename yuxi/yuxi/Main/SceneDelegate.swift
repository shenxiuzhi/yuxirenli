...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        if UserInfo.mr_findFirst()?.number != nil {
            let sceneWindow = UIWindow.init(windowScene: windowScene)
            let tabVc = SMainTabBarController.init()
            sceneWindow.rootViewController = tabVc
            sceneWindow.backgroundColor = .white
            sceneWindow.makeKeyAndVisible()
            self.window = sceneWindow
        }else {
            self.showLoginViewControllerWith()
        }
        
    }
    
    @objc func showLoginViewControllerWith() {
        //登录界面
